...
def test_create_order_expired_jwt_accepted_when_authentication_disabled(
    ...
    """POST /api/v1/orders with expired JWT returns 401 by default, 200 when authentication_disabled."""
    token = jwt.encode(
        {
            "client_ref": "test-client-ref",
            "expiration_at": int(time.time()) - 3600,
    ...
    # With auth disabled we skip expiry check but still validate quote; invalid quote -> 400
